...
describe('locales t()', () => {
  ...
  it('expose les memes cles en en, ru, pt et es que en fr', () => {
    ...
    expect(flattenKeys(es).sort()).toEqual(frKeys)
  })

  it('couvre les cles phase 2 partage et messagerie', () => {
    for (const locale of [fr, en, ru, pt, es]) {
      ...
      expect(locale.legal.sections.mentions.paragraphs.length).toBeGreaterThan(8)
      expect(locale.legal.sections.entreprises.paragraphs.length).toBeGreaterThan(8)
      expect(locale.legal.sections.cgu.paragraphs.length).toBeGreaterThan(6)
      expect(locale.legal.sections.annonces.paragraphs.length).toBeGreaterThan(10)

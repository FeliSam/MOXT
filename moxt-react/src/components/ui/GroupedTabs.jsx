...
export function GroupedTabs({
  ...
  onChange,
  label = 'Sections',
  className = 'grid-cols-1 min-[480px]:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5',
}) {
  const flatTabs = groups.flatMap((group) => group.tabs)

  return (
    <nav className={`grid gap-2.5 sm:gap-3 ${className}`} aria-label={label}>
      {groups.map((group) => (
        <section
          key={group.id}
          className="rounded-[1.25rem] border border-[var(--app-border)] bg-[var(--app-surface)]/80 p-2.5 shadow-[var(--shadow-card)] sm:p-3"
        >
          <h2 className="px-1 text-[10px] font-black uppercase tracking-[0.14em] text-[var(--app-text-faint)]">
            {group.label}
          </h2>
          <div
            className="mt-2 flex flex-wrap gap-1.5 lg:flex-col lg:flex-nowrap"
            role="tablist"
            ...
            {group.tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`inline-flex min-h-9 items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-bold transition-all sm:text-sm lg:w-full lg:justify-start ${
                    isActive

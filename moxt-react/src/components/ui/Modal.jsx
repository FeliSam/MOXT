...
export function Modal({
  ...
  const modal = (
    <div
      className={`fixed inset-0 z-[var(--z-modal)] grid place-items-center ${
        bare ? 'p-0 sm:p-4' : 'p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:p-4'
      }`}
      role="presentation"
    >
      <button
      ...
      <section
        ...
        className={
          bare
            ? `relative z-10 min-w-0 focus:outline-none ${panelClassName}`
            : `scrollbar-hidden relative z-10 max-h-[calc(100dvh-1.5rem)] min-w-0 w-full shadow-2xl sm:max-h-[92vh] ${sizes[size]} ${
                immersive

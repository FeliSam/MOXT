...
import { addVideoComment, deleteVideoComment } from './videosSlice'
...
export function VideoCommentsSheet({ video, open, onClose }) {
              ...
              {comments.map((comment) => {
                ...
                return (
                  <li key={comment.id} className="flex items-start gap-3">
                    ...
                    {(user?.id === comment.authorId || user?.id === video.ownerId || isModerator) &&
                    comment.id ? (
                      <button
                        type="button"
                        onClick={() =>
                          dispatch(deleteVideoComment({ videoId: video.id, commentId: comment.id }))
                        }

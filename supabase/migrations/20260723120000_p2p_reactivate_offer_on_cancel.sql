-- When a P2P order is cancelled, restore the offer to active if the client
-- payload says so (buyer cancel / payment expiry). Archive when requested.
-- Previously the RPC only ever set accepted ← active and never reactivated.
...
declare
  ...
  v_order_status text := coalesce(p_order->>'status', 'created');
  v_offer_status text := coalesce(nullif(v_offer_row->>'status', ''), '');
...
begin
  ...
  if not exists (select 1 from public.p2p_offers where id = v_offer_id) then
    ...
    insert into public.p2p_offers (
      ...
      coalesce(nullif(v_offer_status, ''), 'accepted'),
      case
  ...
  elsif v_order_status = 'cancelled' and v_offer_status = 'active' then
    ...
    where id = v_offer_id
      ...
      and not exists (
        ...
        where o.offer_id = v_offer_id
          and o.id is distinct from v_order_id
          and o.status not in ('cancelled', 'completed')
      );
  elsif v_order_status = 'cancelled' and v_offer_status = 'archived' then
    update public.p2p_offers
    set
      status = 'archived',
      updated_at = now()

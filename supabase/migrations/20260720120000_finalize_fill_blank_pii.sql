-- If phone_verified was set by a client race before finalize, still fill blank
-- identity fields from registration params. Never overwrite non-empty PII.
...
begin
  ...
  -- Already verified: fill blanks only, never clobber existing PII.
  ...
  if found and coalesce(v_row.phone_verified, false) is true then
    ...
    set
      first_name = case
        when length(trim(coalesce(p.first_name, ''))) >= 2 then p.first_name
        else coalesce(
      ...
      last_name = case
        when length(trim(coalesce(p.last_name, ''))) >= 2 then p.last_name
        else coalesce(
      ...
      email = case
        when position('@' in coalesce(p.email, '')) > 0 then p.email
      ...
      origin_phone = case
        when length(trim(coalesce(p.origin_phone, ''))) > 0 then p.origin_phone
        else coalesce(nullif(trim(coalesce(p_origin_phone, '')), ''), coalesce(p.origin_phone, ''))
      end,
      origin_country = case
        when length(trim(coalesce(p.origin_country, ''))) >= 2 then p.origin_country
        else coalesce(
          nullif(trim(coalesce(p_origin_country, '')), ''),
          nullif(trim(coalesce(v_auth.raw_user_meta_data ->> 'origin_country', '')), ''),
          coalesce(p.origin_country, 'BJ')
        )
      end,
      city = case
        when length(trim(coalesce(p.city, ''))) >= 2 then p.city
        else coalesce(
      ...
      avatar_url = case
        when length(trim(coalesce(p.avatar_url, ''))) > 0 then p.avatar_url
        else coalesce(nullif(trim(coalesce(p_avatar_url, '')), ''), coalesce(p.avatar_url, ''))
      end,
      phone = v_phone,
      updated_at = v_now
    where p.id = v_uid

-- Après acceptation acheteur, le vendeur doit confirmer sa disponibilité
-- (seller_accepted) avant que l'acheteur puisse déclarer le paiement.
...
begin
  if new.status is distinct from old.status then
    ...
    if not is_staff then
      ...
      if new.status = 'seller_accepted' then
        if old.status <> 'created' then
          raise exception 'Transition invalide vers seller_accepted depuis %.', old.status;
        end if;
        if actor is distinct from old.seller_id::text then
          raise exception 'Seul le vendeur peut confirmer sa disponibilité.';
        end if;

      elsif new.status = 'waiting_payment' then
        if old.status <> 'seller_accepted' then
        ...
        if old.status not in ('created', 'seller_accepted') then

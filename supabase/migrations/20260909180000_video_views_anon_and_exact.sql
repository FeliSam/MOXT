-- Vues vidéo : compteur exact en base, y compris visiteurs non connectés.
-- Les autres types (listing/job/…) restent réservés aux sessions authentifiées.
...
begin
  ...
  if v_viewer = '' and p_entity_type is distinct from 'video' then
    return null;
  end if;
  ...
  case p_entity_type
    when 'listing' then
      ...
      where id = p_entity_id and owner_id::text is distinct from v_viewer
    ...
    when 'video' then
      ...
      where id = p_entity_id
        and status = 'active'
        and (v_viewer = '' or owner_id::text is distinct from v_viewer)
...
grant execute on function public.moxt_increment_view(text, text) to anon, authenticated;

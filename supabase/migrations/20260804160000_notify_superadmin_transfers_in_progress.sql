-- Notifier les superadmins des transferts en cours sur la plateforme.
...
begin
  ...
  -- Statuts terminaux ignorés à la création ; traités à la mise à jour
  if tg_op = 'INSERT' and new.status in ('cancelled', 'expired', 'declined') then
    return new;
  end if;
  ...
  v_status_label := case new.status
    when 'pending_acceptance' then 'en attente d’acceptation'
    when 'pending' then 'en attente de paiement'
    when 'declared' then 'paiement déclaré'
  ...
  elsif new.status in ('cancelled', 'expired', 'declined') then
...
drop trigger if exists transfers_notify_superadmins on public.transfers;
create trigger transfers_notify_superadmins
  after insert or update of status on public.transfers
  for each row
  execute function public.moxt_notify_superadmins_transfer_progress();

revoke all on function public.moxt_notify_superadmins_transfer_progress() from public, anon;

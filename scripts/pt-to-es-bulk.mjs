#!/usr/bin/env node
/**
 * Convert remaining en-identical es.js keys using Portuguese (pt) as source.
 * Offline PT→ES morphological + lexical conversion; saves maps and applies them.
 *
 * Usage:
 *   node scripts/pt-to-es-bulk.mjs              # convert + write .es-out/pt-bulk.json + apply
 *   node scripts/pt-to-es-bulk.mjs --dry        # stats only
 *   node scripts/pt-to-es-bulk.mjs --no-apply   # write map only
 */
...
const OUT = path.join(ROOT, 'scripts/.es-out/pt-bulk.json')
...
for (const key of Object.keys(enFlat)) {
  ...
  // If PT is also English, try converting EN via light rules only when PT===EN
  const out = ptToEs(src)
  if (out === enFlat[key] || out === src && src === enFlat[key]) {
    // Still identical — keep trying morphology on EN if PT was English
    if (src === enFlat[key]) {
      skippedSame += 1
      continue
    }
...
fs.writeFileSync(OUT, `${JSON.stringify({ translations }, null, 2)}\n`, 'utf8')
console.log('Wrote', OUT)

if (noApply) process.exit(0)
  ...
  JSON.stringify(
    {
      sameAsEn: same,
      different,
      pctDifferent: ((100 * different) / Object.keys(enFlat).length).toFixed(1),
